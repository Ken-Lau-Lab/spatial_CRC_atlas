...
dir.create("./ST_out/CytoTRACE/", showWarnings = FALSE)
```
...
```{r}
for (data_in in list.files("../data/ST/")){
  if (grep("filtered_trimmed", data_in)){
    message(data_in)
    results <- CytoTRACE_fixed(
      prepare_data(paste0("../data/ST/", data_in)),
      subsamplesize = 3000,
      ncores = 2,
      enableFast = TRUE
    )
