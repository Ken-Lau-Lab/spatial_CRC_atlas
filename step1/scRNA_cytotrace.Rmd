---
title: "cytotrace_bob_revised"
output: html_notebook
---

This notebook contains an updated **CytoTRACE** method for compatibility with AnnData objects in `.h5ad` format.

```{r}
#install these 5 libraries
library(nnls)
library(HiClimR)
library(ccaPP)
...
CytoTRACE_fixed <- function(mat, batch = NULL, enableFast = TRUE,
  ...
  message("Transposition thing skipped")                    
  names(cytotrace) <- names(cytotrace_ranked) <- names(gcs) <- names(counts) <- colnames(a1) #<- colnames(mat2)
...
```{r}
for (i in c(
  "Broad_Epi_CRC_NoNormal_ReFiltered_Counts",
  "Broad_NonEpi_sub",
  "VUMC_HTAN_DIS_EPI_V2",
  "VUMC_HTAN_VAL_DIS_NONEPI_V2"
)){
    data_in <- paste0("../data/scRNA/", i, ".h5ad")
    results <- CytoTRACE_fixed(
      prepare_data(data_in),
      subsamplesize = 3000,
      ncores = 2,
      enableFast = TRUE
    )

    write.csv(results$CytoTRACE, paste0("scRNA_out/", i, "_CytoTRACE.csv"))
}
```
